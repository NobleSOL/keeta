...
export function SlippageDialog({ open, onClose }: { open: boolean; onClose: () => void }) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative z-10 w-full max-w-md rounded-2xl border border-border/60 bg-card p-5 shadow-2xl">
        ...
        <p className="mb-4 text-sm text-muted-foreground">
          Slippage is the difference between the current market price and the price when your swap executes.
        </p>
        ...
        <div className="mt-4 text-right">
          <button className="rounded-md bg-brand px-4 py-2 text-sm text-white hover:bg-brand/90" onClick={onClose}>
            Done
          </button>
        </div>
      </div>
...
export default function SlippagePortal() {
  ...
  useEffect(() => {
    ...
    return () => document.removeEventListener("sb:open-slippage", handler as any);
  }, []);
