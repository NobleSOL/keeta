...
contract SilverbackV2Pair {
    ...
    function mint(address to) external lock returns (uint liquidity) {
        ...
        if (totalSupply == 0) {
            ...
            _mint(address(0), MINIMUM_LIQUIDITY); // lock first MINIMUM_LIQUIDITY
        } else {
    ...
    function swap(uint amount0Out, uint amount1Out, address to) external lock {
        ...
        uint amount0In = balance0 + amount0Out > _reserve0 ? balance0 + amount0Out - _reserve0 : 0;
        uint amount1In = balance1 + amount1Out > _reserve1 ? balance1 + amount1Out - _reserve1 : 0;
