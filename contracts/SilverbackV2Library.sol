...
library SilverbackV2Library {
    // returns sorted token addresses, used to handle return values from pairs sorted by address
    ...
    function pairFor(address factory, address tokenA, address tokenB) internal pure returns (address pair) {
        ...
        bytes32 hash = keccak256(abi.encodePacked(
            ...
            factory,
            salt,
            keccak256(type(SilverbackV2Pair).creationCode)
        ));
        ...
        internal view returns (uint reserveA, uint reserveB)
    {
        ...
        (uint reserve0, uint reserve1, ) = ISilverbackV2Pair(pair).getReserves();
